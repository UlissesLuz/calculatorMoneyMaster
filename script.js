document.getElementById('metaForm').addEventListener('submit', function(event) {
    ...
    const valorMeta = parseFloat(document.getElementById('valorMeta').value.replace(/\./g, '').replace(',', '.'));
    ...
    const valorAtingido = calcularMetaFinanceira(valorInicial, contribuicaoPeriodica, frequencia, duracao);
    ...
    const contribuicaoTexto = pluralOuSingular(numeroContribuicoes, periodosFrequencias[frequencia][0], periodosFrequencias[frequencia][1]);
    ...
    if (valorAtingido >= valorMeta) {
        ...
        mensagemFinal = 'Você está muito perto de alcançar sua meta financeira! Com alguns ajustes nas contribuições ou na extimativa de tempo, você pode alcançar o valor exato desejado. Iniciar com um valor maior também pode te ajudar a conquistar sua meta mais rápido.';
    }
    ...
    document.getElementById('resultados').innerHTML = `
        <p>Para atingir a meta de ${formatadorNumerico.format(valorMeta)} em ${duracao} ${duracaoTexto} com contribuições ${contribuicaoTexto}, você precisará contribuir aproximadamente ${formatadorNumerico.format(contribuicaoPeriodica)} por ${frequenciaTexto}.</p>
        <p>${mensagemDuracao}</p>
        <p>${mensagemFinal}</p>
    `;
});
...
const formatarParaDecimal = (input) => {
    input.addEventListener('input', function(event) {
        ...
        valor = (parseFloat(valor) / 100).toFixed(2).replace('.', ',').replace(/\B(?=(\d{3})+(?!\d))/g, ".");
